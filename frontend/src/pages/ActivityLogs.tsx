...
export const ActivityLogs: React.FC = () => {
  ...
  const { data: logsData, isLoading: loading } = useQuery<{
    ...
    queryKey: [
      "activity-logs",
      { page, search: debouncedSearch, actionFilter },
    ],
    queryFn: () =>
      api
        .get("/activity-logs", {
  ...
  const logs = useMemo(() => {
    return rawLogs.filter((log) => {
      const matchesSearch =
        ...
        (log.users?.name &&
          log.users.name.toLowerCase().includes(debouncedSearch.toLowerCase()));
  ...
  useEffect(() => {
    if (page * limit < total) {
      ...
      queryClient.prefetchQuery({
        queryKey: [
          "activity-logs",
          { page: nextPage, search: debouncedSearch, actionFilter },
        ],
        queryFn: () =>
          api
            .get("/activity-logs", {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        <div className="col-span-2 sm:col-span-1 bg-white dark:bg-[#111827] border border-neutral-200 dark:border-neutral-800 p-3.5 rounded-card flex items-center gap-3">
          <div
            className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${hasActiveFilters ? "bg-amber-500/10 text-amber-500" : "bg-neutral-100 dark:bg-neutral-800 text-neutral-400"}`}
          >
        ...
        <EmptyState
          ...
          description={
            hasActiveFilters
              ...
              : "System activity is tracked automatically as changes occur."
          }
          action={
            hasActiveFilters
              ? {
                  ...
                  {logs.map((log) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4.5">
                        <span
                          className={`inline-block font-bold px-2.5 py-1 rounded text-[10px] tracking-wider border uppercase ${getActionBadgeStyle(log.action)}`}
                        >
                          {log.action}
                        </span>
                      </td>
                      ...
                      <td
                        className="px-6 py-4.5 text-neutral-550 dark:text-neutral-350 max-w-sm truncate text-xs font-medium"
                      ...
                      <td className="px-6 py-4.5">
                        <div className="text-xs">
                          ...
                          <p className="text-neutral-400 capitalize text-[10px]">
                            {log.users?.role || "Admin"}
                          </p>
                        </div>
                      </td>
                ...
                <div key={log.id} className="relative">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <span
                        className={`inline-block font-bold px-2 py-0.5 rounded text-[10px] tracking-wider border uppercase ${getActionBadgeStyle(log.action)}`}
                      >
                        {log.action}
                      </span>
                      <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1.5 text-[10px] text-neutral-400 pt-0.5">
                      ...
                      <span className="font-bold text-neutral-500 dark:text-neutral-450">
                        {log.users?.name || "System"}
                      </span>
                      <span className="capitalize text-neutral-400/80">
                        ({log.users?.role || "Admin"})
                      </span>
                    </div>
                  </div>
